#!/bin/sh
# Script cài đặt ainovel-cli (phiên bản Việt hóa) chỉ với một cú nhấp chuột
#
#   curl -fsSL https://raw.githubusercontent.com/aidino/ainovel-cli-vi/main/scripts/install.sh | sh
#   curl -fsSL https://raw.githubusercontent.com/aidino/ainovel-cli-vi/v1.2.3/scripts/install.sh | sh -s -- v1.2.3
#
...
REPO="aidino/ainovel-cli-vi"
BIN="ainovel-cli"
DEST="${AINOVEL_INSTALL_DIR:-/usr/local/bin}"
VERSION="${AINOVEL_VERSION:-${1:-latest}}"

for cmd in curl tar; do
	command -v "$cmd" >/dev/null 2>&1 || { echo "Cần $cmd, vui lòng cài đặt trước rồi thử lại"; exit 1; }
done
...
if command -v sha256sum >/dev/null 2>&1; then
	...
	echo "Cần sha256sum hoặc shasum, vui lòng cài đặt trước rồi thử lại"
	exit 1
fi
...
case "$(uname -s)" in
	Darwin) OS="Darwin" ;;
	Linux)  OS="Linux" ;;
	*) echo "Hệ thống không được hỗ trợ $(uname -s); Windows vui lòng tải xuống thủ công tại https://github.com/$REPO/releases"; exit 1 ;;
esac
...
if [ "$VERSION" = "latest" ] || [ -z "$VERSION" ]; then
	...
	echo "Đang truy vấn phiên bản mới nhất..."
else
	case "$VERSION" in
		v*) TAG="$VERSION" ;;
		*) TAG="v$VERSION" ;;
	...
	echo "Đang truy vấn phiên bản $TAG..."
fi
...
tar -xOzf "$TMP/$ASSET" "$BIN" > "$TMP/extract/$BIN" || {
	echo "Không thể giải nén $BIN từ gói cài đặt, từ chối cài đặt"
	exit 1
}
...
if [ -w "$DEST" ]; then
	...
	echo "Cần quyền quản trị viên để ghi vào $DEST"
	sudo mv "$TMP/extract/$BIN" "$DEST/$BIN"
fi

echo "✓ Cài đặt hoàn tất: $DEST/$BIN"
[ -n "$TAG" ] && echo "Phiên bản: $TAG"
command -v "$BIN" >/dev/null 2>&1 || echo "Gợi ý: $DEST không có trong PATH, vui lòng thêm nó vào PATH"
