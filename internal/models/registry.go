// Package models cung cấp registry metadata model LLM (cửa sổ ngữ cảnh, giới hạn đầu ra, giá),
// nguồn dữ liệu OpenRouter API, baseline biên dịch + làm mới runtime.
package models
...
type ModelEntry struct {
	Provider            string  `json:"provider"`               // tên nhà cung cấp đã chuẩn hóa của OpenRouter (anthropic/openai/gemini/...)
	ID                  string  `json:"id"`                     // ID model (không kèm tiền tố nhà cung cấp)
	Name                string  `json:"name"`                   // tên hiển thị
	ContextWindow       int     `json:"context_window"`         // cửa sổ đầu vào
	MaxTokens           int     `json:"max_tokens"`             // giới hạn đầu ra một lần
	InputCostPer1M      float64 `json:"input_cost_per_1m"`      // giá đầu vào (USD/1M tokens)
...
// ModelRegistry lưu các model đã biết, hỗ trợ phân giải mờ và hợp nhất runtime.
type ModelRegistry struct {
	mu     sync.RWMutex
	models []ModelEntry
}

// NewModelRegistry trả về một registry đã nạp baseline biên dịch.
...
// DefaultRegistry trả về registry toàn cục (lazy load, an toàn luồng).
// Gọi StartPricingRefresh ở giai đoạn khởi động để nền làm mới thông tin giá/cửa sổ.
...
//
// Khi khớp nhiều mục, ưu tiên trả về alias không có hậu tố ngày (ví dụ claude-sonnet-4 ưu tiên hơn claude-sonnet-4-20250514).
func (r *ModelRegistry) Resolve(pattern string) (*ModelEntry, bool) {
	...
	if idx := strings.Index(pattern, "/"); idx > 0 {
		...
		// Tiền tố vendor của OpenRouter (google/, x-ai/) chưa chắc bằng tên Provider nội bộ,
		// lùi về chỉ tra theo modelID để đảm bảo "google/gemini-2.5-pro" khớp được mục gemini.
...
// ResolveContextWindow trả về cửa sổ ngữ cảnh của một model; không khớp trả 0.
...
// Giá/cửa sổ/MaxTokens/Name khác 0 sẽ ghi đè mục sẵn có; mục mới thì thêm thẳng.
