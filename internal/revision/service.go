...
func (s *Service) Sync(ctx context.Context) (*Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("đọc bản ghi phục hồi sửa đổi: %w", err)
	}
	if pending != nil {
	...
	if err := s.validatePendingRecords(items); err != nil {
		return nil, fmt.Errorf("kiểm tra trạng thái chương sau khi sửa đổi: %w", err)
	}
	now := time.Now()
	...
	if err := s.store.Revisions.SavePending(*pending); err != nil {
		return nil, fmt.Errorf("lưu bản ghi phục hồi sửa đổi: %w", err)
	}
...
func (s *Service) applyPending(pending domain.PendingRevision) (*Result, error) {
		...
		if err := s.applyPreparedRecords(pending.Items); err != nil {
			if errors.Is(err, errPreparedStale) {
				if clearErr := s.store.Revisions.ClearPending(); clearErr != nil {
					return nil, fmt.Errorf("%v; dọn dẹp phân tích sửa đổi hết hạn thất bại: %w", err, clearErr)
				}
			}
			return nil, err
		}
	...
	case domain.RevisionStageRecordsApplied:
		...
		if err := NewProjector(s.store).Apply(records); err != nil {
			return nil, fmt.Errorf("tái thiết trạng thái phát sinh của chương: %w", err)
		}
		if len(pending.Items) > 0 {
			if err := s.store.InvalidateChapterAggregates(pending.Items[0].Chapter); err != nil {
				return nil, fmt.Errorf("vô hiệu hóa trạng thái phát sinh cấp cao sau sửa đổi: %w", err)
			}
		}
		for _, item := range pending.Items {
			if slices.Contains(progress.PendingRewrites, item.Chapter) {
				if err := s.store.Progress.CompleteRewrite(item.Chapter); err != nil {
					return nil, fmt.Errorf("hoàn thành làm lại nhân công chương %d: %w", item.Chapter, err)
				}
			}
		}
		for _, item := range pending.Items {
			...
			if analysis.StoryChanged || analysis.OutlineImpact != nil || len(analysis.DownstreamIssues) > 0 {
				...
				if err := s.store.Outline.AppendOutlineFeedback(feedback); err != nil {
					return nil, fmt.Errorf("lưu ảnh hưởng đại cương chương %d: %w", item.Chapter, err)
				}
			}
		}
	...
	case domain.RevisionStageProjectionsApplied:
		for _, item := range pending.Items {
			if _, err := s.store.Checkpoints.AppendArtifact(domain.ChapterScope(item.Chapter), "revision_sync", store.ChapterRecordPath(item.Chapter)); err != nil {
				return nil, fmt.Errorf("ghi lại checkpoint sửa đổi chương %d: %w", item.Chapter, err)
			}
		}
	default:
		return nil, fmt.Errorf("không rõ giai đoạn phục hồi sửa đổi %q", pending.Stage)
	}
	if err := s.store.Revisions.ClearPending(); err != nil {
		return nil, fmt.Errorf("dọn dẹp bản ghi phục hồi sửa đổi: %w", err)
	}
	result := &Result{}
	for _, item := range pending.Items {
...
func (s *Service) applyPreparedRecords(items []domain.PendingRevisionItem) error {
	for _, item := range items {
		...
		if err != nil {
			return fmt.Errorf("đọc chính văn không gian làm việc chương %d: %w", item.Chapter, err)
		}
		if domain.ChapterContentSHA256(content) != item.CurrentSHA256 {
			return fmt.Errorf("%w: chương %d lại thay đổi sau khi phân tích, vui lòng chạy lại /sync", errPreparedStale, item.Chapter)
		}
		...
		default:
			return fmt.Errorf("%w: bản ghi tiếp nhận chương %d đã thay đổi sau khi phân tích, vui lòng chạy lại /sync", errPreparedStale, item.Chapter)
		}
	}
	return nil
}
...
func (s *Service) downstreamSummaries(chapter int, proposed map[int]domain.ChapterSummary) ([]domain.ChapterSummary, error) {
	...
	for _, current := range chapters {
		...
		if summary == nil {
			return nil, fmt.Errorf("chương %d thiếu tóm tắt, không thể phân tích đầy đủ ảnh hưởng tiếp theo", current)
		}
		summaries = append(summaries, *summary)
	}
