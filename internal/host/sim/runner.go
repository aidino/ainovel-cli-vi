...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	go func() {
		...
		if err != nil {
			emit(StageError, 0, len(sources), "Đọc hình mẫu sẵn có thất bại", err)
			return
		}
		pending := pendingSources(existing, sources)
		if len(pending) == 0 {
			emit(StageDone, 0, len(sources), "Hình mẫu đã là mới nhất, không phát hiện bài viết mới hoặc bị thay đổi", nil)
			return
		}
		...
		for i, source := range pending {
			if err := ctx.Err(); err != nil {
				emit(StageError, i, len(pending), "Người dùng hủy phân tích hình mẫu", err)
				return
			}
		...
		emit(StageMerge, len(pending), len(pending), "Ghép hình mẫu mô phỏng...", nil)
		synthesis, err := MergeSynthesis(ctx, deps.LLM, deps.Prompts.Merge, existing, allReports)
		if err != nil {
			emit(StageError, len(pending), len(pending), "Ghép hình mẫu thất bại", err)
			return
		}
		...
		if err := deps.Store.Simulation.Save(profile); err != nil {
			emit(StageError, len(pending), len(pending), "Lưu hình mẫu mô phỏng thất bại", err)
			return
		}
		emit(StageDone, len(pending), len(pending), fmt.Sprintf("Hình mẫu mô phỏng đã cập nhật: thêm/đổi %d bài, lũy kế %d bài", len(pending), len(profile.Corpus.Sources)), nil)
	}()
	return events, nil
}
	...
	out, err := llmcontract.Execute(ctx, model, llmcontract.Request[T]{
		...
		Hooks: llmcontract.Hooks{
			Resolved: func(res llmcontract.Resolution) {
				slog.Debug("Chọn giao thức cấu trúc hóa hình mẫu mô phỏng", "contract", contract.Name,
			...
			Correction: func(ev llmcontract.Correction) {
				slog.Warn("Hình mẫu mô phỏng xuất ra tự phục hồi", "contract", contract.Name, "attempt", ev.Attempt,
