...
// Run thực thi một lần xuất. Trả về đồng bộ, lượng IO nhỏ (đọc ghi file cục bộ).
//
// Ngữ nghĩa thất bại:
//   - deps/opts không hợp lệ → cấu hình sai, trả về ngay
//   - Không có chương nào hoàn thành → trả về lỗi (để bên gọi rõ ràng)
//   - Chương nào đó trong phạm vi chapters/{ch}.md bị thiếu → trả về lỗi (progress không nhất quán với file system là bug tầng sự thật, nên để người dùng thấy)
//   - Đường dẫn đầu ra đã tồn tại và chưa chỉ định Overwrite → trả về lỗi
//
// Skipped dùng cho tình huống "hợp lệ trong phạm vi nhưng chưa hoàn thành" (người dùng truyền to=100 nhưng mới viết đến 80).
func Run(ctx context.Context, deps Deps, opts Options) (*Result, error) {
	...
	for _, ch := range chapters {
		...
		if strings.TrimSpace(text) == "" {
			return nil, fmt.Errorf("progress đánh dấu chương %d đã xong, nhưng chapters/%02d.md thiếu hoặc rỗng", ch, ch)
		}
		bodies[ch] = text
	}
...
// inferFormat suy luận định dạng từ hậu tố đường dẫn đầu ra. Đường dẫn rỗng lui về TXT; hậu tố không xác định thì báo lỗi (tránh lỗi ngầm).
func inferFormat(path string) (Format, error) {
	...
	default:
		return "", fmt.Errorf("Không thể suy luận định dạng từ đuôi %q (hỗ trợ .txt / .epub)", filepath.Ext(path))
	}
}

// atomicWrite giống WriteFile trong store/io.go: tmp + sync + rename.
// Không dùng lại store.IO vì đường dẫn đầu ra có thể nằm ngoài store.Dir().
...
// sanitizeFileName thay thế các ký tự không được phép hoặc dễ nhầm lẫn trên hầu hết các file system trong tên file.
// Không chuyển mã quá quyết liệt, chỉ chặn ký tự phân cách đường dẫn và ký tự điều khiển.
