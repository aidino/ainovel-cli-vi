...
// ProviderSnapshot là cấu hình provider đã được làm sạch dành cho TUI.
...
// ModelConfigurationDraft là bản nháp cấu hình provider đơn lẻ do /config gửi lên Host.
// Chỉ mô tả định nghĩa của provider đó (giao thức/chứng chỉ/thư viện model), không chứa "đang dùng cái nào"——việc chuyển đổi thuộc về /model.
...
// Host chỉ di chuyển các tham chiếu default, nhân vật và fallback khi TUI submit rõ ràng quan hệ này.
...
// MaskAPIKey chỉ giữ lại các đoạn đầu cuối đủ để nhận diện chứng chỉ; chứng chỉ ngắn sẽ bị ẩn toàn bộ.
// TUI chỉ nhận được kết quả này, tuyệt đối không giữ API Key đầy đủ trong cấu hình.
...
// ModelConfiguration trả về cấu hình đã được làm sạch, mục tiêu có thể ghi và tham chiếu model, tuyệt đối không lộ API Key hiện có.
...
// prepareProviderDraftLocked chuẩn hóa bản nháp TUI và gộp vào bản sao cấu hình, lưu và kiểm tra kết nối dùng chung một chuỗi kiểm tra.
func (h *Host) prepareProviderDraftLocked(draft ModelConfigurationDraft) (preparedProviderDraft, error) {
	...
	for _, model := range draft.Models {
		...
		if seen[model.Name] {
			return preparedProviderDraft{}, fmt.Errorf("model %q bị trùng lặp", model.Name)
		}
	...
	case "", APIKeyKeep:
		// Giữ nguyên giá trị hiện có trong cấu hình đề xuất; khi thêm provider mới thì tự nhiên sẽ rỗng.
	case APIKeyReplace:
	...
	if pc.RequiresAPIKey(draft.Provider) && pc.APIKey == "" {
		return preparedProviderDraft{}, fmt.Errorf("Provider %q phải được cấu hình API Key", draft.Provider)
	}
...
// ConfigureModels kiểm tra, lưu trữ và áp dụng nóng thư viện model của một provider.
func (h *Host) ConfigureModels(draft ModelConfigurationDraft) error {
	...
	// Trước khi xóa model, kiểm tra tham chiếu: model được mặc định cấp cao nhất hoặc bất kỳ nhân vật/fallback nào trỏ tới đều không được xóa,
	// yêu cầu người dùng đổi trong /model trước——/config không tự đổi mặc định thay.
	for _, old := range preparedDraft.oldModels {
		...
		if refs := h.modelReferencesLocked(draft.Provider, old.Name); len(refs) > 0 {
			return fmt.Errorf("model %q vẫn đang được %s tham chiếu, vui lòng chuyển đổi trong /model trước khi xóa", old.Name, strings.Join(refs, "、"))
		}
	}

	// Chỉnh sửa thông thường không làm thay đổi "đang dùng cái nào"; đổi tên rõ ràng chỉ di chuyển danh tính tham chiếu của cùng một model.
	...
	h.cfg = candidate
	// Sau khi xây dựng lại client model, đẩy xuống lại cường độ suy luận: applyThinkingLocked sẽ kẹp giá trị hiệu lực theo năng lực model mới của từng nhân vật,
...
func validateModelRenames(requested []ModelRename, oldModels, newModels []bootstrap.ModelConfig) (map[string]string, error) {
	...
	for _, rename := range requested {
		...
		if from == "" || to == "" {
			return nil, fmt.Errorf("tên cũ và tên mới của model khi đổi tên không được để trống")
		}
		if from == to {
			continue
		}
		...
		if _, exists := renames[from]; exists {
			return nil, fmt.Errorf("model %q bị đổi tên nhiều lần", from)
		}
		if targets[to] {
...
func (h *Host) saveModelConfigurationLocked(candidate bootstrap.Config, provider string, pc bootstrap.ProviderConfig, renamed bool) error {
	if renamed {
		// Tham chiếu và định nghĩa provider phải được ghi đĩa trong cùng một lần thay thế tệp, nếu không khi tiến trình khởi động lại có thể chỉ thấy một nửa.
		// /model cũng dùng SaveConfig để ghi lại cấu hình hiệu lực; đổi tên tiếp tục dùng cùng một ngữ nghĩa.
		return bootstrap.SaveConfig(h.configPath, candidate)
	}
	return bootstrap.SaveProviderConfig(h.configPath, provider, pc)
}

// TestModelConnection dùng bản nháp hiện tại cấu tạo một client model thực và gửi yêu cầu tối thiểu.
// Nó không lưu cấu hình, không chuyển đổi model runtime, và cũng không lùi về Provider khác khi thất bại.
