...
//
// Không giống /import sử dụng luồng sự kiện: xuất là đồng bộ IO cục bộ, không có tiến trình trung gian;
// sau khi chạy xong trong goroutine sẽ gửi lại thông báo này một lần.
...
// Việc xuất thực sự chạy trong tea.Cmd (tránh block UI), sau khi hoàn thành sẽ gửi exportDoneMsg.
...
//
// Tham số vị trí: tối đa một, làm đường dẫn đầu ra; mặc định do exp.Run quyết định ({novelDir}/{BookMetadata.Title}.txt).
	...
	for _, a := range args {
		...
		if k, v, ok := strings.Cut(a, "="); ok {
			...
			case "from":
				...
				if err != nil || n < 0 {
					return exp.Options{}, fmt.Errorf("from cần là số nguyên không âm: %q", v)
				}
				opts.From = n
			case "to":
				n, err := strconv.Atoi(v)
				if err != nil || n < 0 {
					return exp.Options{}, fmt.Errorf("to cần là số nguyên không âm: %q", v)
				}
				opts.To = n
			default:
...
// formatExportSuccess render Result thành Event Summary.
