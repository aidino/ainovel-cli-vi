...
func (s *simulationState) refresh(contentW int) {
	...
	case s.err != nil:
		b.WriteString(errStyle.Render("Xử lý hồ sơ viết phỏng thất bại"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc Đóng bảng"))
	default:
		b.WriteString(okStyle.Render("Hồ sơ viết phỏng đã sẵn sàng, Agent phía sau sẽ đọc từ novel_context"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc Đóng bảng"))
	}
...
func renderSimulationModal(width, height int, s *simulationState) string {
	...
	modal := renderPaddedModalFrame(boxW, boxH, "Hồ sơ viết phỏng", hint, strings.Split(s.viewport.View(), "\n"))
...
func startSimulate(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Tạo hồ sơ viết phỏng", "./simulate", width, height, cancel)
...
func startImportSimulation(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Nhập hồ sơ viết phỏng", args[0], width, height, cancel)
	return state, listenSimulationEvent(reqID, ch), nil
}
