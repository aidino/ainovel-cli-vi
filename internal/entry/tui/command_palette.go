...
// syncCommandInputHighlight sử dụng lại sổ đăng ký lệnh để nhận dạng token đầu tiên. Chỉ ghi lệnh hoàn chỉnh, đã đăng ký;
// khi render cuối cùng chỉ tô màu token này, tham số giữ màu chính văn thường.
...
func renderCommandPalette(width int, items []commandPaletteItem, cursor int) string {
	...
	for i, item := range visible {
		...
		// truncateWidth cắt theo chiều rộng hiển thị (ký tự tiếng Trung tính 2 cột); dùng truncate sẽ đếm số rune,
		// trường hợp tiếng Trung chiều rộng thực tế = gấp đôi mong muốn, gây tràn cửa sổ.
