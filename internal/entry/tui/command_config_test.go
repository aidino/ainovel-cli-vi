...
func TestModelRenameProducesExplicitDraftAndReferenceNotice(t *testing.T) {
	...
	if !strings.Contains(st.message, "cập nhật tham chiếu đồng bộ") || !strings.Contains(st.message, "default") {
...
func TestModelListRendersEditableColumnsAndReferences(t *testing.T) {
	...
	for _, want := range []string{"ID mô hình", "Cửa sổ ngữ cản", "Tham chiếu", "deepseek-chat", "128K", "default", "+ Thêm mô hình..."} {
