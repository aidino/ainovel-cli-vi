...
type release struct {
	...
	Body    string         `json:"body"` // nguyên văn release notes (markdown), dùng hiển thị nhắc nhở kiểm tra phiên bản
...
// verifyChecksum kiểm tra danh mục SHA256 do GoReleaser sinh ra. Danh mục và gói cài đặt phải đến từ cùng một
// release; thiếu mục, sai định dạng hoặc digest không khớp đều từ chối thay file thực thi hiện tại.
func verifyChecksum(archivePath, checksumPath, assetName string) error {
	...
	if expected == "" {
		return fmt.Errorf("không tìm thấy %s trong danh mục checksum", assetName)
	}
	...
	if actual != expected {
		return fmt.Errorf("%s SHA256 kiểm tra thất bại: got %s, expected %s", assetName, actual, expected)
	}
	return nil
}
...
func replaceExecutable(dst, src string) (string, error) {
	if dst == "" {
		return "", fmt.Errorf("không thể xác định vị trí tệp thực thi hiện tại")
	}
