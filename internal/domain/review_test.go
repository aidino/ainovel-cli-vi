...
func TestRestoreOwnPlants(t *testing.T) {
	cases := []struct {
		...
		{
			name: "重写把本chương  plant 写成 advance：plant 补回队首",
		...
		{
			name: "重写把本chương  plant 整条丢了：仍补回",
			prev: []ForeshadowUpdate{{ID: "f_photo", Action: "plant", Description: "泄洪道旧照"}},
			next: nil,
			want: []ForeshadowUpdate{{ID: "f_photo", Action: "plant", Description: "泄洪道旧照"}},
		},
		{
			name: "新记录已自dòng 声明 plant：不lặp lại 补",
			prev: []ForeshadowUpdate{{ID: "f_photo", Action: "plant", Description: "旧mô tả "}},
			next: []ForeshadowUpdate{{ID: "f_photo", Action: "plant", Description: "新mô tả "}},
			want: []ForeshadowUpdate{{ID: "f_photo", Action: "plant", Description: "新mô tả "}},
		},
		{
		...
		{
			name: "多条 plant 按原序补回，và都排在 advance 之前",
			prev: []ForeshadowUpdate{
