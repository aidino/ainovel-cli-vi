...
// LoadSnapshots đọc ảnh chụp nhân vật của tập và arc chỉ định.
...
// LoadLatestSnapshots tải ảnh chụp nhân vật gần nhất (tìm kiếm ngược theo tập và arc).
func (s *CharacterStore) LoadLatestSnapshots() ([]domain.CharacterSnapshot, error) {
	// Thiếu đại cương→(nil,nil) đã được xử lý trong LoadLayeredOutline; hỏng hóc phải ném ra, nếu không ảnh chụp nhân vật
	// sẽ biến mất một cách âm thầm cùng với việc hỏng đại cương phân tầng, quá trình sáng tác mất đi tính liên tục của nhân vật mà không ai biết.
