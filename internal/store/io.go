...
// IO đóng gói các thao tác đọc ghi hệ thống tệp, cung cấp cơ chế khóa và ghi nguyên tử.
// Mỗi sub-store giữ một phiên bản IO độc lập, sở hữu sync.RWMutex riêng.
type IO struct {
	dir string
	mu  sync.RWMutex
}
...
// WriteMarkdownUnlocked ghi ra .md sidecar. Quy ước: mỗi .md là khung nhìn có thể đọc
// nỗ lực tốt nhất (best-effort) tương ứng của .json, tuyệt đối không phải nguồn dữ liệu——thời gian chạy và xuất ra đều được hiển thị lại từ .json.
// Các phương thức Save ghi .json trước rồi ghi .md này trong cùng một khóa ghi, là hai lần tmp+rename độc lập;
// Sự cố giữa hai bước sẽ để lại .md tụt hậu so với .json, điều này là có thể chấp nhận (không ai coi .md là dữ liệu để đọc,
// lần sau ghi cùng một scope sẽ tự phục hồi). Cố tình không thêm cam kết nguyên tử hai tệp cho việc này——đó là thiết kế quá mức.
...
// syncFileUnlocked xác nhận bản ghi nối thêm đã tồn tại được bền bỉ hóa trong quá trình phát lại lũy đẳng.
