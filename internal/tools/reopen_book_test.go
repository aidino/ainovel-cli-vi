...
// completedBook 构造一本已hoàn kết 的 N chương tiểu thuyết （phase=complete，CompletedChapters=1..n）。
...
func TestReopenBookReopensCompletedBook(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{"chapters": []int{3, 1}, "reason": "dọn dẹp 特殊chữ 符"})
	...
	if len(p.PendingRewrites) != 2 || p.PendingRewrites[0] != 3 || p.PendingRewrites[1] != 1 {
		t.Errorf("PendingRewrites = %v, want [3 1] (vào hàng chờ nguyên bản)", p.PendingRewrites)
	}
...
func TestReopenBookRejectsNonCompleteBook(t *testing.T) {
	// sáng tác 中（未hoàn kết ）的书不能 reopen
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}
