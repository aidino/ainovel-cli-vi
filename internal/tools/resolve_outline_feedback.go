...
func (t *ResolveOutlineFeedbackTool) Description() string {
	return "Xác nhận đã xem xét toàn bộ writer_feedback, và kế hoạch sau hiện tại vẫn phù hợp. Chỉ gọi khi không cần sửa đại cương; cần sửa thì dùng revise_outline hoặc công cụ cấu trúc."
}
...
func (t *ResolveOutlineFeedbackTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if len(feedback) == 0 {
		return nil, fmt.Errorf("không có phản hồi đại cương chờ xử lý: %w", errs.ErrToolPrecondition)
	}
