...
func (t *ContextTool) Description() string {
	return "Lấy trạng thái hiện tại và ngữ cảnh sáng tác của tiểu thuyết." +
		"Không truyền chapter: trả về progress_status (các trường tiến độ phase/flow/next_chapter/pending_rewrites) + tổng quan quy hoạch tinh gọn, dùng để xác định bước tiếp theo；" +
		"Architect trường thiên có thể truyền volume + arc để đọc chi tiết chương của một arc đã triển khai." +
...
func (t *ContextTool) Schema() map[string]any {
	return schema.Object(
		schema.Property("chapter", schema.Int("Số chương. Không truyền thì trả về trạng thái tiến độ và thiết lập nền tảng (dùng cho Architect); truyền vào thì trả thêm ngữ cảnh viết của chương đó (Writer/Editor dùng)")),
		schema.Property("volume", schema.Int("Tùy chọn Architect trường thiên: số tập cần đọc tập trung; phải truyền cùng arc, không dùng cùng chapter")),
		schema.Property("arc", schema.Int("Tùy chọn Architect trường thiên: số arc trong tập cần đọc tập trung; phải truyền cùng volume, không dùng cùng chapter")),
	)
}
...
func (t *ContextTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if a.Chapter > 0 {
		...
		if epi, ok := result["episodic_memory"].(map[string]any); ok && len(epi) > 0 {
			epi["_usage"] = "Khối này là bản ghi sự kiện của nội dung đã viết (dùng để đối chiếu nhất quán và liên kết); sao chép nguyên văn các nội dung này trong chương mới là lỗi trùng lặp"
		}
	} else {
	...
	// Tầng tool chỉ thực hiện lựa chọn ngữ nghĩa liên quan đến nhiệm vụ; dung lượng ngữ cảnh do từng Worker quản lý theo cửa sổ model thực tế.
